...
BEGIN
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE WARNING 'Service Role Key가 설정되지 않았습니다. 스케줄 생성 전에 설정해주세요.';
      v_service_role_key := '';
  END;
  
  -- Service Role Key가 있으면 스케줄 생성
  IF v_service_role_key != '' THEN
    ...
    RAISE NOTICE '마감일 임박 알림 스케줄이 생성되었습니다: 매일 UTC 00:00 (KST 09:00)';
  ELSE
    RAISE WARNING 'Service Role Key가 설정되지 않아 마감일 임박 알림 스케줄을 생성하지 않습니다.';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE WARNING 'Service Role Key가 설정되지 않아 마감일 초과 알림 스케줄을 생성하지 않습니다.';
  END IF;
END $$;
...
--    - 파일 내 'YOUR_SUPABASE_PROJECT_URL_HERE'를 실제 프로젝트 URL로 변경
--    - Supabase Dashboard > Settings > API > Project URL에서 확인 가능
